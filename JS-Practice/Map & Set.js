...
// let names = ['John','Mark','Jerry'];

// let newArr = names.map(function(name){
//     return this.greeting(this.phrase,name)
// },obj1);
...
let map1=new Map();
map1.set("apple","red");
map1.set("grapes","green ")
console.log(map1);

function getOrDefault(map,key,value=2) {
    map.set(key,value);
    //console.log(map)
}

let pops=new Map();
getOrDefault(pops,'music',"kinfg");
getOrDefault(pops,'hin');

console.log(pops)

// Question 
let words=['abc','def','ghi','jkl','abc','ghi'];
function countWordsUsingMap(arr){
    let map = new Map();
    for (const word of arr) {
        if(!map.has(word)){
            map.set(word,1)
...
let eatables = [{type:'fruit',name: 'Apple'},{type:'vegetable',name: 'Tomato'},{type:'fruit',name: 'orange'}];

let z = eatables.reduce((acc,item)=>{
    let key = item['type'];
...
let map2= new Map();
map2.set('mango','yellow');
map2.set('apple','purple');
...
// Question 
let exampleObj = Object.fromEntries(map3);
